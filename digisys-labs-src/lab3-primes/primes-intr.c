...
//2574 millisec for 8192
//11253 millisec for 1
...
static volatile int max_bufcnt = 0;
...
//eg.
// buffer = empty, hardware sending last character, txidle = 0
//call serial_putc(ch), see txidle = 0 so prepare to go to else branch
//interrupt happens after checking condition but before going to else branch. UART_TXDRDY = 1 just finished sending, call uart handler
//handler checks buffer, hasn't put ch into buffer yet, so bufcnt == 0. there is nothing to send, so txidle = 1
//buf_put(ch) is called
//ch sits in the buffer indefinitely, hardware is asleep and will not look for it


//if only cover buf_put(ch); txidle will have a problem, as before
...
void init(void)
{
    ...
    printf("Max buffer usage: %d chars\n", max_bufcnt);
    //Max buffer usage: 6709 chars, 2590 millisec
    stop_timer();
}
