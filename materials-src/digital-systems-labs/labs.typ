...
#set heading(numbering: (..nums) => {
  ...
  // Apply your offset: Level 1 becomes (value - 1)
  // Subsequent levels stay as they are
  let adjusted = values.enumerate().map(((i, v)) => {
    if i == 0 { v - 1 } else { v }
  })

  if values.len() == 1 {
    // Top level heading
    [Lab #str(adjusted.at(0)).]
  } else {
    // Lower level headings (e.g., 0.1, 1.2.1)
    adjusted.map(str).join(".")
  }
})

// Reset counter to 0 so the first heading starts at "Lab 0"
#counter(heading).update(0)
...
The purpose of this lab is to setup our computer environment so that we are able to compile, run, and test the programs we write. Somewhat annoyingly, this can be sometimes be more difficult than the actual programming itself! Thankfully, the labs are set up in such a way that many of the details are abstracted away. Nevertheless, you will need to develop some "comptuer use" skills in order to become effective at writing programs. Some students will already be fully comfortable with this, while others will never have used computers in this way before. For this course, and beyond it, you will need to pick up these skills, even though they will not be formally taught. This lab does try to help by providing some explicit guidance. However, for everyone, these skills are to an extent self-taught through practice, internet searches, and exchanges with colleagues.#footnote[The informal nature by which these important skills are developed has been noted by many in the field, and thankfully some have put some effort into collating things together in a short course called "#link("https://missing.csail.mit.edu/")[The Missing Semester of Your CS Education]". It is worth checking this out if you want to develop these skills further.] During labs, you can ask the demonstrators for help, but I also encourage you to discuss with your fellow students. If have these skills, I encourage you to be helpful to your fellow students, when asked.

In this lab, we will:
+ Learn how to use the Linux command line.
+ Set up our environment and text editor.
